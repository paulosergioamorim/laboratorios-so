...
int main()
{
    ...
    { /* ***child*** */
        ...
        printf("\t My PID=%ld \tMy father's PID=%ld \tShell's PID=%ld\n", (long int)ch_pid, (long int)father_pid,
               (long int)shell_pid);
        printf("\t My GID=%d \tMy father's GID=%d \tShell's GID=%d\n", getpgid(ch_pid), getpgid(father_pid),
               getpgid(shell_pid));
        printf("\t My SID=%d \tMy father's SID=%d \tShell's SID=%d\n", getsid(ch_pid), getsid(father_pid),
               getsid(shell_pid));
        sleep(1);
        printf("\nNote that my father and I are both in a Foreground group... accordingly, we receive any SIGNAL from "
               "the Controlling Terminal. For ex: if you do \"Ctrl-c\" we both recive a signal SIGINT and die! Try it "
               "or press [ENTER]...\n");
